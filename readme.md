This program is used in the paper *AUTOMATA AND FINITE ORDER ELEMENTS IN THE NOTTINGHAM GROUP, by JAKUB BYSZEWSKI, GUNTHER CORNELISSEN, AND DJURRE TIJSMA*, for finding all 2-automata on at most 5 vertices generating a power series `\sigma = x+O(x^2)` of compositional order 2 or 4.
...
Note, to every 2-automaton we associate a power series in `x` with coefficients from the finite field of 2 elements. The coefficient `a_k` in the power series `\sigma(x) = x + a_2 x^2 + a_3 x^3 + ...` is equal to the label of the final vertex of the walk that starts at the start vertex and follows the edges according to the successive digits of `k` in base `2`. 
