...
- The power series generated by the automaton satisfies `a_0 = 0` and `a_1 = 1`, i.e. it starts with `\sigma = x + x^2 + O(x^3)`.
- The power series has compositional order `2` or `4`, i.e. if `\sigma(x)=x+x^2+...` then `\sigma^{\circ 2}(x)=\sigma(x)+\sigma(x)^2+...`. 
